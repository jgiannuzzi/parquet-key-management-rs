...
use parquet_key_management::kms::{KmsClientFactory, KmsClientRef, KmsConnectionConfig};
    ...
    Fut: Future<Output = Result<()>>,
{
    ...
    let crypto_factory =
        CryptoFactory::new_async(spawner, TestAsyncKmsClientFactory::with_default_keys());
    ...
    Fut: Future<Output = Result<()>>,
{
    ...
    let write_crypto_factory =
        CryptoFactory::new_async(spawner.clone(), write_client_factory.clone());
    let read_crypto_factory =
        CryptoFactory::new_async(spawner.clone(), read_client_factory.clone());
