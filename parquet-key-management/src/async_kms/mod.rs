...
impl KmsConnectionConfig {
    ...
    pub async fn key_access_token(&self) -> String {
        self.key_access_token.read().await.clone()
    }

    /// Return the key access token inside a read lock.
    pub(crate) async fn read_key_access_token(&self) -> RwLockReadGuard<'_, String> {
        self.key_access_token.read().await
    }
    ...
    pub async fn refresh_key_access_token(&self, key_access_token: String) {
        let mut token = self.key_access_token.write().await;
