use crate::async_kms::{KmsClientFactory, KmsClientRef, KmsConnectionConfig};
...
impl KmsManager {
    ...
    pub async fn get_client(
        ...
        self.clear_expired_entries(cache_lifetime).await;
    ...
    pub async fn get_kek_read_cache(
        &self,
        kms_connection_config: &Arc<KmsConnectionConfig>,
        cache_lifetime: Option<Duration>,
    ) -> KekReadCache {
        self.clear_expired_entries(cache_lifetime).await;
        let key = KekCacheKey::new(kms_connection_config.key_access_token().await.clone());
    ...
    pub async fn get_kek_write_cache(
        &self,
        kms_connection_config: &Arc<KmsConnectionConfig>,
        cache_lifetime: Option<Duration>,
    ) -> KekWriteCache {
    ...
    async fn clear_expired_entries(&self, cleanup_interval: Option<Duration>) {
        if let Some(cleanup_interval) = cleanup_interval {
            self.kms_client_cache.clear_expired(cleanup_interval).await;
            self.kek_read_caches.clear_expired(cleanup_interval).await;
            self.kek_write_caches.clear_expired(cleanup_interval).await;
        }
    }
    ...
    pub async fn cache_stats(&self) -> CacheStats {
        CacheStats {
            num_kms_clients: self.kms_client_cache.cache.lock().await.len(),
            num_kek_read_caches: self.kek_read_caches.cache.lock().await.len(),
            num_kek_write_caches: self.kek_write_caches.cache.lock().await.len(),
        }
    }
}
    ...
    TValue: Clone,
{
    ...
    where
        F: FnOnce() -> Fut,
        Fut: Future<Output = Result<TValue>>,
    {
    ...
    pub async fn clear_expired(&self, cleanup_interval: Duration) {
        {
