...
impl<'a> KeyWrapper<'a> {
    pub async fn new(
        ...
        let master_key_to_kek = kms_manager
            .get_kek_write_cache(
    ...
    pub async fn get_key_metadata(
        ...
        let key_material = if self.encryption_configuration.double_wrapping() {
            let mut master_key_to_kek = self.master_key_to_kek.lock().await;
            ...
            let kms_client = self
                ...
                .await?;
            let wrapped = kms_client.wrap_key(key, master_key_id).await?;
            key_material_builder
...
async fn generate_key_encryption_key(
    master_key_id: &str,
    ...
    let kms_client = kms_manager
        .get_client(kms_connection_config, cache_lifetime)
        .await?;
    let wrapped_key = kms_client.wrap_key(&key, master_key_id).await?;
