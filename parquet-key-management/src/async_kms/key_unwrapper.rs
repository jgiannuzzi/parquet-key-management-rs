use crate::async_kms::kms_manager::{KekReadCache, KmsManager};
...
use parquet::encryption::decrypt::AsyncKeyRetriever;
...
impl KeyUnwrapper {
    ...
    async fn unwrap_single_wrapped_key(
        &self,
        wrapped_dek: &str,
        master_key_id: &str,
    ) -> Result<Vec<u8>> {
        let kms_connection_config = self.kms_connection_config.read().await;
        let client = self
            .kms_manager
            .get_client(
                &kms_connection_config,
            ...
            .await?;
        client.unwrap_key(wrapped_dek, master_key_id).await
    }

    async fn unwrap_double_wrapped_key(
        &self,
        wrapped_dek: &str,
        master_key_id: &str,
        kek_id: &str,
        ...
        let mut kek_cache = self.kek_cache.lock().await;
        let kek = match kek_cache.entry(kek_id.to_owned()) {
            ...
            Entry::Vacant(entry) => {
                ...
                let client = self
                    ...
                    .await?;
                let kek = client.unwrap_key(wrapped_kek, master_key_id).await?;
                entry.insert(kek)
            }
        };
    ...
    async fn update_kms_config_from_footer_metadata(
        &self,
        kms_instance_id: &str,
        kms_instance_url: &str,
    ) -> Result<()> {
        let mut kms_connection_config = self.kms_connection_config.write().await;
...
impl AsyncKeyRetriever for KeyUnwrapper {
    fn retrieve_key<'a>(&'a self, key_metadata: &'a [u8]) -> BoxFuture<'a, Result<Vec<u8>>> {
        async move {
            ...
            if key_material.double_wrapping {
                ...
                self.unwrap_single_wrapped_key(
                    &key_material.wrapped_dek,
                    &key_material.master_key_id,
                )
                .await
